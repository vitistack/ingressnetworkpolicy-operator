...
IMG ?= ncr.sky.nhn.no/ghcr/vitistack/ingressnetworkpolicy-operator:latest
...
.PHONY: test
test: test-setup ## Run tests. ## Run tests.
...
KIND_CLUSTER ?= ingressnetworkpolicy-operator-test-e2e
...
docker-buildx: ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx create --name ingressnetworkpolicy-operator-builder
	$(CONTAINER_TOOL) buildx use ingressnetworkpolicy-operator-builder
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG} -f Dockerfile.cross .
	- $(CONTAINER_TOOL) buildx rm ingressnetworkpolicy-operator-builder
	rm Dockerfile.cross
